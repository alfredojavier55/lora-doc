...
```{r}
lira_prob <- read.csv(file = "ranges3.csv", sep = ";")
# lira_prob <- lira_prob[-7,]
# lira_prob <- lira_prob[-6,]
lira_prob$Range <- cut(lira_prob$range, breaks = lira_prob$range, include.lowest = FALSE)
datatable(lira_prob)
```
...
# Resume
## Numeric results by disease annual prevalence and categories
```{r}
# datatable(wt %>% 
#   group_by(disease, date, species) %>%
#   summarise(mean_prevalence=round(mean(prev, na.rm = TRUE),6)))

wt2 <- wt %>% 
  group_by(disease, date, species) %>%
  summarise(m=as.numeric(round(mean(prev, na.rm = TRUE),6)),
            md=as.numeric(round(median(prev, na.rm = TRUE),6)))

wt2$range <- cut(wt2$m, breaks = lira_prob$range, include.lowest = TRUE)
...
wt %>% 
  group_by(disease, date, species) %>%
  ggplot(aes(prev, color=disease, group=date))+
  geom_boxplot()+
  facet_grid(vars(disease), vars(date), scales = "free_x") +
  theme(legend.position="none")+
  labs(title = "Prevalence by years")
...
wt %>% 
  ...
  scale_x_log10() +
  theme(legend.position="none")+
  labs(title = "Prevalence by years (log scale)")


wt %>% 
  group_by(disease, date, species) %>%
  ggplot(aes(prev, color=disease)) +
  geom_boxplot()+
  facet_wrap(vars(disease), ncol = 1) +
  theme_minimal() +
  theme(legend.position="none")+
  labs(title = "Prevalence by disease")

wt %>% 
  group_by(disease, date, species) %>%
  ggplot(aes(prev, color=disease)) +
  geom_boxplot()+
  facet_wrap(vars(disease), ncol = 1, scales = "free_x") +
  theme_minimal() +
  theme(legend.position="none") +
  labs(title = "Prevalence by disease (scale free)")
...
wt %>% 
  ...
  theme_minimal() +
  theme(legend.position="none") +
  labs(title = "Prevalence by disease (log scale)")
...
write.csv(st, file = "summary.mean.csv")
